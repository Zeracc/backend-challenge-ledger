...
| Transações de aposta          | Em andamento | `OPENING`, `BET`, `WIN` e `LOSS`; referências e reversões permanecem planejadas                               | Unidade, rejeição auditável e PostgreSQL real            |
| Ledger                        | Em andamento | `CREDIT` de abertura/WIN, `DEBIT` de BET e ausência em LOSS, com imutabilidade e unicidade                    | Constraints, rollback e reconciliação após cada cenário  |
...
| API HTTP                      | Em andamento | `POST /wallets` e `POST /wagering/transactions` para BET, WIN e LOSS; consultas e readiness planejados        | E2E de sucesso, replay, conflito, rejeição e validação   |
...
| Outbox transacional           | Em andamento | Eventos de OPENING, BET, WIN e LOSS incluídos na transação; publisher e leases permanecem planejados          | Rollback integral de débito e crédito em falha de outbox |
